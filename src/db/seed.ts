...
function seed() {
  ...
  for (const g of genreNames) insertGenre.run(g);
  const genreRow = (name: string) =>
    db.prepare("SELECT id FROM genres WHERE name = ?").get(name) as {
      id: number;
    };

  // ---- Screens + Seats ----
  const screenCount = db.prepare("SELECT COUNT(*) as c FROM screens").get() as {
    c: number;
  };
  if (screenCount.c === 0) {
    const insertScreen = db.prepare(
      "INSERT INTO screens (name, rows, columns) VALUES (?, ?, ?)",
    );
    ...
    const createScreenWithSeats = transaction(
    ...
    for (const s of screens) createScreenWithSeats(s.name, s.rows, s.columns);
  ...
  if (movieCount.c === 0) {
    ...
    const movies = [
      {
        ...
        description:
          "An elite team races to stop a rogue AI from taking control of global satellites.",
      ...
      },
      {
        ...
        description:
          "A retired fisherman confronts a secret from his past when a stranger arrives in town.",
      ...
      },
      {
        title: "Punchline",
        description:
          "Two rival stand-up comedians are forced to co-write a show for one night only.",
      ...
      },
      {
        title: "Hollow House",
        description:
          "A family moves into a house with a history that refuses to stay buried.",
    ...
    const insertMovieTx = transaction((m: (typeof movies)[number]) => {
    ...
    const screens = db.prepare("SELECT id FROM screens").all() as {
      id: number;
    }[];
    ...
    const movieDurations = db
      .prepare("SELECT id, duration_minutes FROM movies")
      .all() as { id: number; duration_minutes: number }[];
    const durationMap = new Map(
      movieDurations.map((m) => [m.id, m.duration_minutes]),
    );
