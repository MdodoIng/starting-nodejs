...
exports.getRevenueByMovie = getRevenueByMovie;
exports.getCapacityReport = getCapacityReport;
const database_1 = require("../../db/database");
/** Overall summary: total revenue, total reservations, total tickets sold. */
function getSummary() {
    const revenue = database_1.db
        .prepare(`SELECT COALESCE(SUM(total_amount), 0) as total_revenue FROM reservations WHERE status = 'confirmed'`)
        .get();
    const reservationCount = database_1.db
        .prepare(`SELECT COUNT(*) as count FROM reservations WHERE status = 'confirmed'`)
        .get();
    const ticketsSold = database_1.db
        .prepare(`SELECT COUNT(*) as count FROM reservation_seats rs
       JOIN reservations r ON r.id = rs.reservation_id
       WHERE r.status = 'confirmed'`)
        .get();
    const upcomingShowtimes = database_1.db
        .prepare(`SELECT COUNT(*) as count FROM showtimes WHERE start_time > datetime('now')`)
        .get();
    return {
...
function getRevenueByMovie(filters) {
    let query = `
...
function getCapacityReport(filters) {
    let query = `
    SELECT
      s.id as showtime_id,
  ...
  `;
    ...
    return rows.map((row) => ({
        ...row,
        utilization_percent: row.total_seats > 0
